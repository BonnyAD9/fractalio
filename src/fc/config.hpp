...
public:
    ...
    FontSet font_sort(
        Pattern &pat, bool trim, FcCharSet **csp, FcResult &result
    ) {
        auto res = FcFontSort(
            get(), pat.get(), trim ? FcTrue : FcFalse, csp, &result
        );
        if (!res) {
    ...
    Pattern font_render_prepare(Pattern &pat, FcPattern &font) {
        auto res = FcFontRenderPrepare(get(), pat.get(), &font);
        if (!res) {
            throw std::runtime_error("Couldn't prepare font for rendering.");
        }
        return { res };
    }
